  ...
  <el-card class="main-card">
    ...
    <el-dialog :visible.sync="addOrEdit" width="40%" top="3vh">
      ...
      <el-form label-width="80px" size="medium" :model="article">
        ...
        <!-- 卡片类型 -->
        <el-form-item label="原文地址" v-if="article.type == 2">
          <el-input
            v-model="article.originalUrl"
            placeholder="请填写原文链接"
          />
        </el-form-item>
         <el-form-item label="卡片备注" v-if="article.type == 3">
           提交后再编辑相册
        </el-form-item>
         <el-form-item label="卡片备注" v-if="article.type == 4">
           提交后再编辑商品信息
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    saveArticleDraft() {
      ...
      if(this.article.type == 3 || this.article.type == 4 && this.article.originalUrl){
        this.article.originalUrl = this.article.type + '';
      }
    ...
    saveOrUpdateArticle() {
      ...
      if (this.article.tagNameList.length == 0) {
        this.$message.error("卡片标签不能为空");
        return false;
      }
      if (this.article.articleCover.trim() == "") {
        this.$message.error("卡片封面不能为空");
        return false;
      }
      ...
      this.axios.post("/api/admin/articles", this.article).then(({ data }) => {
        if (data.flag) {
          ...
          this.timer = setInterval(()=>{
            this.times--
            if(this.times===0){
               this.$router.push('/albums/' + this.article.id);
              if(this.article.type == 3){
                // 跳相册编辑
                 
              }else if( this.article.type == 4){
                // 跳文章编辑
                 this.$router.push('/goods/' + this.article.id);
              }else{
                // 跳文章列表
