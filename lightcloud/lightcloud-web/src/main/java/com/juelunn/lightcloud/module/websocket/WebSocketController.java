package com.juelunn.lightcloud.module.websocket;

import cn.hutool.core.date.DateUtil;
import com.alibaba.fastjson.JSON;
import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;

import com.juelunn.common.enums.ChatTypeEnum;
import com.juelunn.common.enums.FilePathEnum;

import com.juelunn.common.dto.ChatRecordDTO;
import com.juelunn.common.dto.RecallMessageDTO;
import com.juelunn.common.dto.WebsocketMessageDTO;
import com.juelunn.common.entity.ChatRecord;
import com.juelunn.common.util.BeanCopyUtils;
import com.juelunn.common.util.HTMLUtils;
import com.juelunn.common.util.IpUtils;
import com.juelunn.common.vo.VoiceVO;
import com.juelunn.lightcloud.module.blog.dao.ChatRecordDao;
...
public class WebSocketController {
    ...
    public void setChatRecordDao(ChatRecordDao chatRecordDao) {
        WebSocketController.chatRecordDao = chatRecordDao;
    }

    @Autowired
    public void setUploadStrategyContext(UploadStrategyContext uploadStrategyContext) {
        WebSocketController.uploadStrategyContext = uploadStrategyContext;
    }
    ...
    private ChatRecordDTO listChartRecords(EndpointConfig endpointConfig) {
        // 获取聊天历史记录
        List<ChatRecord> chatRecordList = chatRecordDao.selectList(
                new LambdaQueryWrapper<ChatRecord>()
                .ge(ChatRecord::getCreateTime, DateUtil.offsetHour(new Date(), -24)));
        //如果12小时内没人说话，就默认取前面10条数据
        if(chatRecordList.size() == 0){
            chatRecordList = chatRecordDao.selectList(
                    new LambdaQueryWrapper<ChatRecord>().orderByDesc(ChatRecord::getCreateTime).last("limit 0,10"));
            //取到的是倒的时间，需要再逆转下给前台
    ...
    private void broadcastMessage(WebsocketMessageDTO messageDTO) throws IOException {
        for (WebSocketController webSocketService : webSocketSet) {
